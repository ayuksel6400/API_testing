...
response body'sinin asagida verilen ile ayni oldugunu test ediniz
 ...
 Response body = Expected Body
   ...
   */
    @Test
    public  void get01(){
        //1-Endpoint hazırladık
        specJsonPlace.pathParams("pp1","posts","pp2",22);

        //2-Expected Data hazırladık
        TestDataJSONPlace testDataJSONPlace = new TestDataJSONPlace();
        JSONObject expData = testDataJSONPlace.expBodyOlusturJSON();

        //3-Response kaydı hazorlıyoruz
        Response response = given().spec(specJsonPlace).when().get("/{pp1}/{pp2}");

        //4- Assert hazırlıyoruz.
        JsonPath respJS = response.jsonPath();
        assertEquals(testDataJSONPlace.okStatusKodu,response.getStatusCode());
        assertEquals(expData.get("userId"),respJS.get("userId"));
        assertEquals(expData.get("id"),respJS.get("id"));
        assertEquals(expData.get("title"),respJS.get("title"));
        assertEquals(expData.get("body"),respJS.get("body"));
